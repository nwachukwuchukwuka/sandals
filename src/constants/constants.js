import bg_image1 from "../assets/bg_image1.webp";
import bg_image2 from "../assets/bg_image2.webp";
import bg_image4 from "../assets/bg_image4.webp";
import bg_image5 from "../assets/bg_image5.webp";



import speciality from '../assets/speciality.svg'
import padi from '../assets/padi.svg'
import snorkeling from '../assets/snorkeling.svg'
import water_sports from '../assets/water_sports.svg'
import land_sports from '../assets/land_sports.svg'
import championship from '../assets/championship.svg'
import all_tips from '../assets/all_tips.svg'
import premium from '../assets/premium.svg'
import premium_wines from '../assets/premium_wines.svg'
import bars_resorts from '../assets/bars_resorts.svg'
import round_trips from '../assets/round_trips.svg'

import award from "../assets/award.webp";
import award_logo_1 from "../assets/award_logo_1.svg";
import award_logo_2 from "../assets/award_logo_2.png";
import award_logo_3 from "../assets/award_logo_3.svg";
import award_logo_4 from "../assets/award_logo_4.avif";
import award_logo_5 from "../assets/award_logo_5.avif";


// export const background_images = [bg_image1, beach_2, beach_3, beach_4, beach_5, beach_6, beach_7];
export const background_images = [bg_image1, bg_image2, bg_image4, bg_image5];

export const sandals_vacation_images = [
  { image: speciality, content: "Specialty Restaurants" },
  { image: padi, content: "PADI-Certified cuba Diving" },
  { image: snorkeling, content: "Snorkeling Trips" },
  { image: water_sports, content: "Water Sports" },
  { image: land_sports, content: "Land Sports" },
  { image: championship, content: "Championship Golf" },
  { image: all_tips, content: "All Tips, Taxes and Gratuities" },
  { image: premium, content: "Premium Liquors" },
  { image: premium_wines, content: "Premium Wines" },
  { image: bars_resorts, content: "Up to 13 Bars per Resort" },
  { image: round_trips, content: "Roundtrip Airport Transfers" },
]

export const departingFlight = [
  "SAN ANDRES, COLOMBIA(COLOMBIA)",
  "ALLENTOWN, PENNSYLVANIA(PA), USA (ABE)",
  "ABILENE, TEXAS(TX), USA (ABI)",
  "ALBUQUERQUE, NEW MEXICO(NM), USA (ABQ)",
  "ABERDEEN, SOUTH DAKOTA(SD), USA (ABR)",
  "ACAPULCO, GUERRERO(GUERRERO), MEXICO",
  "NANTUCKET, MASSACHUSETTS(MA), USA (ACK)",
  "WACO, TEXAS(TX), USA (ACT)",
  "ATLANTIC CITY, NEW JERSEY(NJ), USA (ACY)",
  "ADELAIDE, SOUTH AUSTRALIA(SOUTH AUSTRALIA)",
  "SAN ANDRES, COLOMBIA(COLOMBIA)",
  "ALEXANDRIA, LOUISIANA(LA), USA (AEX)"
];



export const carousel_data = [
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_sgo_465ec21762.jpg",
    place: "Jamaica",
    name: "Sandals Ochi",
    subName: "Ocho Rios",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "13362",
    price: "199",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_smb_007cf85eb9.jpg",
    place: "Jamaica",
    name: "Sandals Montego Bay",
    subName: "Montego Bay",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "12458",
    price: "209",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "Jamaica",
    name: "Sandals Royal Caribbean",
    subName: "Montego Bay",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "11234",
    price: "219",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "Jamaica",
    name: "Sandals Negril",
    subName: "Negril",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "10987",
    price: "189",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "Jamaica",
    name: "Sandals South Coast",
    subName: "White House",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "9876",
    price: "229",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "Saint Vincent",
    name: "Sandals Saint Vincent",
    subName: "Saint Vincent",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "8765",
    price: "249",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "CURAÇAO",
    name: "Sandals Royal Curaçao",
    subName: "Willemstad",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "9543",
    price: "259",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "SAINT LUCIA",
    name: "Sandals Grande St. Lucian",
    subName: "Gros-Islet",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "11234",
    price: "239",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "BAHAMAS",
    name: "Sandals Royal Bahamian",
    subName: "Nassau",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "10567",
    price: "269",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "ANTIGUA",
    name: "Sandals Grande Antigua",
    subName: "St. John's",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "12345",
    price: "279",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "GRENADA",
    name: "Sandals Grenada",
    subName: "St. George's",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "9876",
    price: "289",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "GRENADA",
    name: "Sandals Grenada",
    subName: "St. George's",
    text: "ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "9876",
    price: "289",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "GRENADA",
    name: "Sandals Grenada",
    subName: "St. George's",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "9876",
    price: "289",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "GRENADA",
    name: "Sandals Grenada",
    subName: "St. George's",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "9876",
    price: "289",
    resortLink: "view resort",
    roomLink: "view room"
  },
  {
    url: "https://cdn.sandals.com/sandals/v13/images/EN/uploads/landscape_swh_3306c3a8e0.jpg",
    place: "BARBADOS",
    name: "Sandals Barbados",
    subName: "St. Lawrence Gap",
    text: "lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.",
    reviews: "10234",
    price: "299",
    resortLink: "view resort",
    roomLink: "view room"
  }
];


export const reviewsCarouselItems = [
  {
    id: 1,
    hotelName: "Sandals Grande Antigua",
    location: "Saint Johns - Antigua and Barbuda",
    rating: 5,
    description: "Paradise with new friends",
    review: "Had an amazing time! joy the bartender at the small pool was the most amazing and attentive person ever. The food was great and the staff was very friendly. I will definitely be back.",
    timeAgo: "1 MONTH AGO ON TRIPADVISOR"
  },
  {
    id: 2,
    hotelName: "Sandals Royal Caribbean",
    location: "Montego Bay - Jamaica",
    rating: 5,
    description: "Sandals Luxury Resort & Private Island",
    review: "Exceptional service and stunning overwater bungalows. The private island experience was unforgettable. Perfect for honeymoons and romantic getaways.",
    timeAgo: "2 WEEKS AGO ON TRIPADVISOR"
  },
  {
    id: 3,
    hotelName: "Sandals Emerald Bay",
    location: "Great Exuma - Bahamas",
    rating: 5,
    description: "Sandals Golf Resort & Spa",
    review: "World-class golfing experience with breathtaking ocean views. The spa treatments were divine and the staff went above and beyond to ensure our comfort.",
    timeAgo: "3 WEEKS AGO ON TRIPADVISOR"
  },
  {
    id: 4,
    hotelName: "Sandals Ochi",
    location: "Ocho Rios - Jamaica",
    rating: 5,
    description: "Sandals All-Inclusive Mountain Resort",
    review: "Amazing hillside retreat with countless dining options. The private pool sanctuaries were a highlight. Perfect blend of relaxation and entertainment.",
    timeAgo: "2 MONTHS AGO ON TRIPADVISOR"
  },
  {
    id: 6,
    hotelName: "Sandals Royal Barbados",
    location: "St. Lawrence Gap - Barbados",
    rating: 5,
    description: "Sandals Luxury All-Inclusive Resort",
    review: "Innovative restaurants and rooftop pool made our stay memorable. The bowling alley was a unique touch. Service was impeccable throughout our stay.",
    timeAgo: "3 MONTHS AGO ON TRIPADVISOR"
  },
  {
    id: 7,
    hotelName: "Sandals Royal Barbados",
    location: "St. Lawrence Gap - Barbados",
    rating: 5,
    description: "Sandals Luxury All-Inclusive Resort",
    review: "Innovative restaurants and rooftop pool made our stay memorable. The bowling alley was a unique touch. Service was impeccable throughout our stay.",
    timeAgo: "3 MONTHS AGO ON TRIPADVISOR"
  },
  {
    id: 8,
    hotelName: "Sandals Royal Barbados",
    location: "St. Lawrence Gap - Barbados",
    rating: 5,
    description: "Sandals Luxury All-Inclusive Resort",
    review: "Innovative restaurants and rooftop pool made our stay memorable. The bowling alley was a unique touch. Service was impeccable throughout our stay.",
    timeAgo: "3 MONTHS AGO ON TRIPADVISOR"
  },
  {
    id: 9,
    hotelName: "Sandals Royal Barbados",
    location: "St. Lawrence Gap - Barbados",
    rating: 5,
    description: "Sandals Luxury All-Inclusive Resort",
    review: "Innovative restaurants and rooftop pool made our stay memorable. The bowling alley was a unique touch. Service was impeccable throughout our stay.",
    timeAgo: "3 MONTHS AGO ON TRIPADVISOR"
  },
  {
    id: 10,
    hotelName: "Sandals Royal Barbados",
    location: "St. Lawrence Gap - Barbados",
    rating: 5,
    description: "Sandals Luxury All-Inclusive Resort",
    review: "Innovative restaurants and rooftop pool made our stay memorable. The bowling alley was a unique touch. Service was impeccable throughout our stay.",
    timeAgo: "3 MONTHS AGO ON TRIPADVISOR"
  },
];

export const awardsCarouselData = [
  {
    logo: award_logo_1,
    background: award,
    title: ["BEST INCLUDED AMENITY", "READERS' CHOICE", "SCUBA DIVING"],
    subtitle: "ISLANDS ALL-INCLUSIVE AWARDS 2020",
  },
  {
    logo: award_logo_2,
    background: award,
    title: ["BEST INCLUDED AMENITY", "READERS' CHOICE", "SCUBA DIVING"],
    subtitle: "ISLANDS ALL-INCLUSIVE AWARDS 2020",
  },
  {
    logo: award_logo_3,
    background: award,
    title: ["BEST INCLUDED AMENITY", "READERS' CHOICE", "SCUBA DIVING"],
    subtitle: "ISLANDS ALL-INCLUSIVE AWARDS 2020",
  },
  {
    logo: award_logo_4,
    background: award,
    title: ["BEST INCLUDED AMENITY", "READERS' CHOICE", "SCUBA DIVING"],
    subtitle: "ISLANDS ALL-INCLUSIVE AWARDS 2020",
  },
  {
    logo: award_logo_5,
    background: award,
    title: ["BEST INCLUDED AMENITY", "READERS' CHOICE", "SCUBA DIVING"],
    subtitle: "ISLANDS ALL-INCLUSIVE AWARDS 2020",
  },
];


export const footerLinks = [
  {
    title: "Book a Vacation",
    links: [
      { text: "Book Online", href: "#" },
      { text: "Get a Price Quote", href: "#" },
      { text: "View Specials", href: "#" },
      { text: "Find All-Inclusive Resort", href: "#" },
      { text: "Find a Travel Agent", href: "#" },
      { text: "Sandals Specialists", href: "#" },
    ],
  },
  {
    title: "Already Booked?",
    links: [
      { text: "Check-in Online", href: "#" },
      { text: "Book Optional Extras", href: "#" },
      { text: "Balance Payment", href: "#" },
      { text: "Butler Preferences", href: "#" },
    ],
  },
  {
    title: "All-Inclusive Resorts",
    links: [
      { text: "View All Resorts", href: "#" },
      { text: "Saint Vincent Resorts", href: "#" },
      { text: "Jamaica Resorts", href: "#" },
      { text: "Saint Lucia Resorts", href: "#" },
      { text: "Antigua Resorts", href: "#" },
      { text: "Bahamas Resorts", href: "#" },
    ],
  },
  {
    title: "Weddings & Celebrations",
    links: [
      { text: "Destination Weddings", href: "#" },
      { text: "Wedding Venues", href: "#" },
      { text: "Inquire", href: "#" },
      { text: "Honeymoons", href: "#" },
      { text: "Elopements", href: "#" },
      { text: "Anniversaries", href: "#" },
    ],
  },
  {
    title: "About Sandals",
    links: [
      { text: "Destination Weddings", href: "#" },
      { text: "Wedding Venues", href: "#" },
      { text: "Inquire", href: "#" },
      { text: "Honeymoons", href: "#" },
      { text: "Elopements", href: "#" },
      { text: "Anniversaries", href: "#" },
      { text: "Accessibility", href: "#" },
      { text: "News & Media", href: "#" },
      { text: "Affiliates", href: "#" },
      { text: "Site Map", href: "#" },
    ],
  },
];