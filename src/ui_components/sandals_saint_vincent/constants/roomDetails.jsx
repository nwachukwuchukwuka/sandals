import room_img_1 from "../../../assets/sandals_saint_vincent/room_img_1.avif";
import room_img_2 from "../../../assets/sandals_saint_vincent/room_img_2.avif";
import category_location_img from "../../../assets/sandals_saint_vincent/category_location_img.jpg";

import room_card_img_1 from "../../../assets/sandals_saint_vincent/room_card_img_1.webp";
import room_card_img_2 from "../../../assets/sandals_saint_vincent/room_card_img_2.avif";
import room_card_img_3 from "../../../assets/sandals_saint_vincent/room_card_img_3.webp";
import room_card_img_4 from "../../../assets/sandals_saint_vincent/room_card_img_4.webp";
import room_card_img_5 from "../../../assets/sandals_saint_vincent/room_card_img_5.avif";
import room_card_img_6 from "../../../assets/sandals_saint_vincent/room_card_img_6.avif";
import room_card_img_7 from "../../../assets/sandals_saint_vincent/room_card_img_7.avif";
import room_card_img_8 from "../../../assets/sandals_saint_vincent/room_card_img_8.avif";
import room_card_img_9 from "../../../assets/sandals_saint_vincent/room_card_img_9.avif";
import room_card_img_10 from "../../../assets/sandals_saint_vincent/room_card_img_10.avif";
import room_card_img_11 from "../../../assets/sandals_saint_vincent/room_card_img_11.avif";
import room_card_img_12 from "../../../assets/sandals_saint_vincent/room_card_img_12.webp";
import room_card_img_13 from "../../../assets/sandals_saint_vincent/room_card_img_13.avif";
import room_card_img_14 from "../../../assets/sandals_saint_vincent/room_card_img_14.avif";
import room_card_img_15 from "../../../assets/sandals_saint_vincent/room_card_img_15.avif";
import room_card_img_16 from "../../../assets/sandals_saint_vincent/room_card_img_16.avif";
import room_card_img_17 from "../../../assets/sandals_saint_vincent/room_card_img_17.webp";
import room_card_img_18 from "../../../assets/sandals_saint_vincent/room_card_img_18.webp";
import room_card_img_19 from "../../../assets/sandals_saint_vincent/room_card_img_19.webp";
import room_card_img_20 from "../../../assets/sandals_saint_vincent/room_card_img_20.avif";
import room_card_img_21 from "../../../assets/sandals_saint_vincent/room_card_img_21.avif";
import room_card_img_22 from "../../../assets/sandals_saint_vincent/room_card_img_22.avif";
import room_card_img_23 from "../../../assets/sandals_saint_vincent/room_card_img_23.avif";
import room_card_img_24 from "../../../assets/sandals_saint_vincent/room_card_img_24.avif";
import room_card_img_25 from "../../../assets/sandals_saint_vincent/room_card_img_25.avif";
import room_card_img_26 from "../../../assets/sandals_saint_vincent/room_card_img_26.avif";
import room_card_img_27 from "../../../assets/sandals_saint_vincent/room_card_img_27.avif";
import room_card_img_28 from "../../../assets/sandals_saint_vincent/room_card_img_28.webp";
import room_card_img_29 from "../../../assets/sandals_saint_vincent/room_card_img_29.avif";
import room_card_img_30 from "../../../assets/sandals_saint_vincent/room_card_img_30.avif";
import room_card_img_31 from "../../../assets/sandals_saint_vincent/room_card_img_31.avif";
import room_card_img_32 from "../../../assets/sandals_saint_vincent/room_card_img_32.webp";
import room_card_img_33 from "../../../assets/sandals_saint_vincent/room_card_img_33.avif";
import room_card_img_34 from "../../../assets/sandals_saint_vincent/room_card_img_34.avif";
import room_card_img_35 from "../../../assets/sandals_saint_vincent/room_card_img_35.avif";
import room_card_img_36 from "../../../assets/sandals_saint_vincent/room_card_img_36.avif";
import room_card_img_37 from "../../../assets/sandals_saint_vincent/room_card_img_37.avif";
import room_card_img_38 from "../../../assets/sandals_saint_vincent/room_card_img_38.avif";
import room_card_img_39 from "../../../assets/sandals_saint_vincent/room_card_img_39.avif";
import room_card_img_40 from "../../../assets/sandals_saint_vincent/room_card_img_40.webp";
import room_card_img_41 from "../../../assets/sandals_saint_vincent/room_card_img_41.avif";
import room_card_img_42 from "../../../assets/sandals_saint_vincent/room_card_img_42.avif";
import room_card_img_43 from "../../../assets/sandals_saint_vincent/room_card_img_43.avif";
import room_card_img_44 from "../../../assets/sandals_saint_vincent/room_card_img_44.avif";
import room_card_img_45 from "../../../assets/sandals_saint_vincent/room_card_img_45.avif";

import {
  BusFront,
  Bath,
  Accessibility,
  Wine,
  HandPlatter,
  Ship,
  ChefHat,
  Tv,
  Dumbbell,
  Bell,
  Waves,
  User,
} from "lucide-react";
import { MdPool } from "react-icons/md";
export const roomDetails = [
  {
    id: "bamboo-palm-balcony",
    images: [
      { src: room_card_img_1, style: "object-cover w-full h-full" },
      { src: room_card_img_2, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_3, style: "h-[150px] object-cover w-full" },
    ],
    images2: [room_card_img_1, room_card_img_2, room_card_img_3],
    title: "Bamboo Palm Room with Balcony Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: LX | Room Views: Pool, Tropical Garden",
    description2: "Room Views: Pool, Tropical Garden",
    details:
      "Located on the second and third floor, embrace your home away from home in this 580 sq. ft. room, with lush garden views and the ultimate seclusion. Take in the inspiring views while enjoying the outdoor Tranquility Soaking Tub or lounge on your sprawling balcony and let this magical island lull you into serenity.",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tub",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <Accessibility />,
        text: "Handicap Friendly",
      },
      {
        icon: <Wine />,
        text: "In-Room Local Beer & Wine",
      },
    ],
    price: "$404",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "bamboo-palm-partial-ocean",
    images: [
      { src: room_card_img_4, style: "object-cover w-full h-full" },
      { src: room_card_img_5, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_6, style: "h-[150px] object-cover w-full" },
    ],
    images2: [
      room_card_img_4,
      room_card_img_5,
      room_card_img_6,
      room_card_img_5,
      room_card_img_6,
    ],
    title:
      "Bamboo Palm Partial Oceanview Room with Balcony Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: PLX | Room Views: Partial Oceanview, Pool, Tropical Garden",
    description2: "Room Views: Partial Oceanview, Pool, Tropical Garden",
    details:
      "Located on the fourth and fifth floor, this 580 sq. ft. elevated enclave has a spacious living room, walk-in closet and a wide balcony so you can enjoy sights of St. Vincent's flora and fauna. When you're ready to wind down, the outdoor Tranquility Tub is there for a relaxing soak at the end of the day.",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tub",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <Wine />,
        text: "In-Room Local Beer & Wine",
      },
    ],
    price: "$425",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "lady-palm-club-media",
    images: [
      { src: room_card_img_7, style: "object-cover w-full h-full" },
      { src: room_card_img_8, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_9, style: "h-[150px] object-cover w-full" },
    ],
    images2: [
      room_card_img_7,
      room_card_img_8,
      room_card_img_9,
      room_card_img_8,
      room_card_img_9,
    ],
    title:
      "Lady Palm Club Suite with Media Room and Balcony Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: CBM | Room Views: Pool, Tropical Garden",
    description2: "Room Views: Pool, Tropical Garden",
    details:
      "Find your Zen in this beautiful 741 sq. ft. room with coastal design. Lounge on the open balcony and gaze at the glittering pool. Take a relaxing soak on your Tranquility Soaking Tub and admire the view of the flourishing landscape.",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tub",
      },
      {
        icon: <Tv />,
        text: "In-Suite Media Room",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <Accessibility />,
        text: "Handicap Friendly",
      },
      {
        icon: <Bell />,
        text: "Concierge Service",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Select Spirits",
      },
    ],
    price: "$457",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "lady-palm-club-fitness",
    images: [
      { src: room_card_img_10, style: "object-cover w-full h-full" },
      { src: room_card_img_11, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_12, style: "h-[150px] object-cover w-full" },
    ],
    images2: [
      room_card_img_10,
      room_card_img_11,
      room_card_img_12,
      room_card_img_11,
      room_card_img_12,
    ],
    title:
      "Lady Palm Club Suite with Fitness Room and Balcony Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: CBE | Room Views: Pool, Tropical Garden",
    description2: "Room Views: Pool, Tropical Garden",
    details:
      "Located on the third and fourth floors, this 741 sq. ft. suite is equipped to handle your fitness needs while you vacation. The suite is adorned with textures and art inspired by the St. Vincent coastline. A new addition is the personal Technogym fitness room with one exercise bike, complete with programs you can personalize.",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tub",
      },
      {
        icon: <Dumbbell />,
        text: "In-Suite Fitness Room",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <Ship />,
        text: "Airport Transfers",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <Bell />,
        text: "Concierge Service",
      },

      {
        icon: <Wine />,
        text: "In-Room Bar w/ Select Spirits",
      },
    ],
    price: "$475",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "lady-palm-partial-ocean-media",
    images: [
      { src: room_card_img_13, style: "object-cover w-full h-full" },
      { src: room_card_img_14, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_15, style: "h-[150px] object-cover w-full" },
    ],
    images2: [
      room_card_img_13,
      room_card_img_14,
      room_card_img_15,
      room_card_img_14,
      room_card_img_15,
    ],
    title:
      "Lady Palm Partial Oceanview Club Suite with Media Room and Balcony Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: PCBM | Room Views: Partial Oceanview, Pool, Tropical Garden",
    description2: "Room Views: Partial Oceanview, Pool, Tropical Garden",
    details:
      "This 741 sq. ft. sublime retreat offers a glimpse of the calming azure waters. You can relax indoors, too, from the comfort of your own media room, complete with a sound bar and 75\" HD TV. On the open balcony, you'll find the ultimate way to unwind: a Tranquility Soaking Tub framed with views of the rich environment.",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <Tv />,
        text: "In-Suite Media Room",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <Bell />,
        text: "Concierge Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Select Spirits",
      },
    ],
    price: "$495",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "bamboo-palm-club-swim-up",
    images: [
      { src: room_card_img_16, style: "object-cover w-full h-full" },
      { src: room_card_img_17, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_18, style: "h-[150px] object-cover w-full" },
    ],
    images2: [
      room_card_img_16,
      room_card_img_17,
      room_card_img_18,
      room_card_img_17,
      room_card_img_18,
    ],
    title: "Bamboo Palm Club Swim-up Room with Patio Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: SUP | Room Views: Pool, Tropical Garden",
    description2: "Room Views: Pool, Tropical Garden",
    details:
      "Located on the ground floor, this 580 sq. ft. relaxing retreat is a little piece of paradise. A light filled space with airy, coastal decor. Take a dip straight from your balcony in your own semi-private pool. Feel the ease wash over you while recharging in your own Tranquility Soaking Tub.",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <Waves />,
        text: "Swim-up Pool",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <Bell />,
        text: "Concierge Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Select Spirits",
      },
    ],
    price: "$537",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "lady-palm-club-swim-up-media",
    images: [
      { src: room_card_img_19, style: "object-cover w-full h-full" },
      { src: room_card_img_20, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_21, style: "h-[150px] object-cover w-full" },
    ],
    images2: [
      room_card_img_19,
      room_card_img_20,
      room_card_img_21,
      room_card_img_20,
      room_card_img_21,
    ],
    title:
      "Lady Palm Club Swim-up Suite with Media Room and Patio Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: 1SUP | Room Views: Pool, Tropical Garden",
    description2: "Room Views: Pool, Tropical Garden",
    details:
      'Experience the serenity of St. Vincent from the comfort of your 741 sq. ft. suite. Recline in the chaise lounge beside the zero-entry swim-up pool and admire the exotic tropical gardens. Enjoy some time out in your own personal media room, equipped with a 75" LED TV and soundbar.',
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <Waves />,
        text: "Swim-up Pool",
      },
      {
        icon: <Tv />,
        text: "In-Suite Media Room",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <Bell />,
        text: "Concierge Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Select Spirits",
      },
    ],
    price: "$558",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "garden-butler-suite",
    images: [
      { src: room_card_img_22, style: "object-cover w-full h-full" },
      { src: room_card_img_23, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_24, style: "h-[150px] object-cover w-full" },
    ],
    images2: [
      room_card_img_22,
      room_card_img_23,
      room_card_img_24,
      room_card_img_23,
      room_card_img_24,
    ],
    title: "Garden Butler Suite with Patio Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: VG | Room View: Tropical Garden",
    description2: "Room View: Tropical Garden",
    details:
      "Experience your vacation uninterrupted. Overlooking the beautiful tropical gardens, the interior of this 889 sq. ft. Butler Suite embraces the light and airy colors of the coast while the suns warmth beams in.",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <User />,
        text: "Butler Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Premium Liquors",
      },
    ],
    price: "$627",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "one-bedroom-garden-butler",
    images: [
      { src: room_card_img_25, style: "object-cover w-full h-full" },
      { src: room_card_img_26, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_27, style: "h-[150px] object-cover w-full" },
    ],
    images2: [
      room_card_img_25,
      room_card_img_26,
      room_card_img_27,
      room_card_img_26,
      room_card_img_27,
    ],
    title: "One-Bedroom Garden Butler Suite with Patio Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: VF | Room View: Tropical Garden",
    description2: "Room View: Tropical Garden",
    details:
      "Breathtaking views of tropical gardens make this 745 sq. ft. Butler Suite the perfect secluded spot. This one-bedroom suite features a separate living room and a private patio with a Tranquility Soaking Tub ideal for unwinding outdoors.",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <Ship />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <User />,
        text: "Butler Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Premium Liquors",
      },
    ],
    price: "$635",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "one-bedroom-garden-butler-media",
    images: [
      { src: room_card_img_28, style: "object-cover w-full h-full" },
      { src: room_card_img_29, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_30, style: "h-[150px] object-cover w-full" },
    ],
    images2: [
      room_card_img_28,
      room_card_img_29,
      room_card_img_30,
      room_card_img_29,
      room_card_img_30,
    ],
    title:
      "One-Bedroom Garden Butler Suite with Media Room and Patio Tranquility Soaking Tub",
    description:
      "Resort: Sandals Saint Vincent | Category Code: VH | Room View: Tropical Garden",
    description2: "Room View: Tropical Garden",
    details:
      "This 1,382 sq. ft. one-bedroom villa is surrounded by lush tropical foliage and offers ample indoor space plus an outdoor Tranquility Soaking Tub. The media room offers plush seating, a HD TV and sound bar for picture-perfect entertainment.",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <Tv />,
        text: "In-Suite Media Room",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <User />,
        text: "Butler Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Premium Liquors",
      },
    ],
    price: "$721",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "beachfront-butler-villa",
    images: [
      { src: room_card_img_31, style: "object-cover w-full h-full" },
      { src: room_card_img_32, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_33, style: "h-[150px] object-cover w-full" },
    ],
    images2: [room_img_1, room_img_2, room_img_2, room_img_2, room_img_2],
    title: "Beachfront Butler Villa Suite with Private Pool",
    description:
      "Resort: Sandals Saint Vincent | Category Code: BVGP | Room Views: Beachfront, Oceanview, Tropical Garden",
    description2: "Room Views: Beachfront, Oceanview, Tropical Garden",
    details:
      "Wake to the sound of waves and admire the soothing beach views from your patio. This magical 1,114 sq. ft. Butler Suite features a spacious patio complete with a Tranquility Soaking Tub, private pool, seating area, and lounge chairs.",
    icons: [
      {
        icon: <Waves />,
        text: "Private Pool",
      },
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },

      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <User />,
        text: "Butler Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Premium Liquors",
      },
    ],
    price: "$860",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "beachfront-one-bedroom-butler",
    images: [
      { src: room_card_img_34, style: "object-cover w-full h-full" },
      { src: room_card_img_35, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_36, style: "h-[150px] object-cover w-full" },
    ],
    images2: [room_img_1, room_img_2, room_img_2, room_img_2, room_img_2],
    title: "Beachfront One-Bedroom Butler Villa Suite with Private Pool",
    description:
      "Resort: Sandals Saint Vincent | Category Code: BVIP | Room Views: Beachfront, Oceanview, Tropical Garden",
    description2: "Room Views: Beachfront, Oceanview, Tropical Garden",
    details:
      "Wake inside this 1,723 sq. ft. villa to the sound of lapping waves and feel the ocean breeze on your skin. Immerse yourself in your own private pool and enjoy the luxury of direct beach access, perfect to celebrate the sunset or the magic of night time star gazing. The in-room bar, comfortable amenities and tropical gardens make it easy to forget the world outside.",
    icons: [
      {
        icon: <Waves />,
        text: "Private Pool",
      },
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <User />,
        text: "Butler Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Premium Liquors",
      },
    ],
    price: "$1,221",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "beachfront-one-bedroom-butler-media",
    images: [
      { src: room_card_img_37, style: "object-cover w-full h-full" },
      { src: room_card_img_38, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_39, style: "h-[150px] object-cover w-full" },
    ],
    images2: [room_img_1, room_img_2, room_img_2, room_img_2, room_img_2],
    title:
      "Beachfront One-Bedroom Butler Villa Suite with Media Room and Private Pool",
    description:
      "Resort: Sandals Saint Vincent | Category Code: BVHP | Room Views: Beachfront, Oceanview, Tropical Garden",
    description2: "Room Views: Beachfront, Oceanview, Tropical Garden",
    details:
      'Offering mesmerizing views of the horizon, this spectacular 1,407 sq. ft. Butler Villa is one of a kind. With direct access to the beach, your own private pool and a dedicated media room complete with a 75" HD TV and sound bar, you have the perfect set up for a movie night. Take a dip in your private pool or saunter over to the beach.',
    icons: [
      {
        icon: <Waves />,
        text: "Private Pool",
      },
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <Tv />,
        text: "In-Suite Media Room",
      },

      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <User />,
        text: "Butler Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Premium Liquors",
      },
    ],
    price: "$1,250",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "beachfront-two-bedroom-butler",
    images: [
      { src: room_card_img_40, style: "object-cover w-full h-full" },
      { src: room_card_img_41, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_42, style: "h-[150px] object-cover w-full" },
    ],
    images2: [room_img_1, room_img_2, room_img_2, room_img_2, room_img_2],
    title: "Beachfront Two-Bedroom Butler Villa Suite with Private Pool",
    description:
      "Resort: Sandals Saint Vincent | Category Code: BVIP | Room Views: Beachfront, Oceanview, Tropical Garden",
    description2: "Room Views: Beachfront, Oceanview, Tropical Garden",
    details:
      "Set in the most idyllic landscape, this spacious 1,631 sq. ft. villa is nestled in prime position. Your vacation will be truly unforgettable with luxuries such as your own private pool and outdoor Tranquility Soaking Tub to take in the cerulean views.",
    icons: [
      {
        icon: <Waves />,
        text: "Private Pool",
      },
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },
      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <User />,
        text: "Butler Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Premium Liquors",
      },
    ],
    price: "$1,298",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },

  {
    id: "vincy-overwater-two-story",
    images: [
      { src: room_card_img_43, style: "object-cover w-full h-full" },
      { src: room_card_img_44, style: "h-[150px] object-cover w-full" },
      { src: room_card_img_45, style: "h-[150px] object-cover w-full" },
    ],
    images2: [room_img_1, room_img_2, room_img_2, room_img_2, room_img_2],
    title: "Vincy Overwater Two-Story Villa",
    description:
      "Resort: Sandals Saint Vincent | Category Code: OWV | Room Views: Beachfront, Oceanfront, Oceanview",
    description2: "Room Views: Beachfront, Oceanfront, Oceanview",
    details:
      "These 1,248 sq. ft. two-story water villas are perfectly set within our magical private cove. With lush green rainforest to the rear and ocean views to the front, you can truly immerse yourself in the sights and sounds of the surf and wildlife. Relax on your day bed which hangs above water, or take your ocean gazing to the dedicated second floor deck. For a private retreat, these villas are nothin...",
    icons: [
      {
        icon: <Bath />,
        text: "Soaking Tubs",
      },

      {
        icon: <HandPlatter />,
        text: "In-Room Dining",
      },
      {
        icon: <BusFront />,
        text: "Airport Transfers",
      },
      {
        icon: <User />,
        text: "Butler Service",
      },
      {
        icon: <Wine />,
        text: "In-Room Bar w/ Premium Liquors",
      },
    ],
    price: "$1,614",
    qualifications: [
      "Get Up to $1000 Instant Credit",
      "Service Personnel Discount",
      "Book Online & Get $100 Credit",
      "Get Up to 55% Off Rack Rate",
      "Last Minute Travel Deals",
    ],
    categoryLocationImage: category_location_img,
  },
];
